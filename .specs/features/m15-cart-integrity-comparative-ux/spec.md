...
O `M13` definiu o ciclo principal `Carrinho -> Checkout -> Pedido -> Treino`, e o `M14` passou a tornar esse showcase visível no catálogo e na aba `Análise`. Mesmo assim, ainda restam três gaps que reduzem a credibilidade da experiência:

1. `CartApplicationService.addItem()` aceita qualquer produto existente e só `OrderApplicationService.createOrder()` valida `available_in` contra o país do cliente, então itens incompatíveis entram no carrinho e falham tarde demais.
2. `ClientSelectorDropdown.tsx` ainda constrói o `Client` selecionado com `totalOrders: 0` e `recentProducts: []`, o que faz o `ClientProfileCard` exibir um falso estado vazio apesar de a API já expor `purchaseSummary` e histórico de pedidos.
3. `ModelStatusPanel` já possui estados `promoted/rejected/failed`, mas a copy ainda é genérica demais e o caso `rejected` pode parecer bug, porque `Pos-Efetivar` pode ficar visualmente igual ao modelo atual sem explicação suficiente.

O `M15` fecha esses gaps finais de integridade e UX comparativa: o carrinho deve respeitar o contexto do cliente antes do checkout, o perfil do cliente deve refletir dados reais de pedidos, e os resultados de `promoted/rejected/failed` devem ser compreensíveis sem inspeção de logs ou código.

## Goals

- [ ] Bloquear produtos incompatíveis com o país do cliente já no `Adicionar ao Carrinho`, antes do checkout
- [ ] Manter feedback coerente entre backend e frontend para ações inválidas de carrinho, sem mensagens contraditórias ou genéricas
- [ ] Enriquecer o cliente selecionado com dados reais de pedidos para o `ClientProfileCard`
- [ ] Tornar os estados `promoted`, `rejected`, `failed` e `unknown` claros para o avaliador no fluxo `Com Carrinho -> Pos-Efetivar`
- [ ] Preservar degradação graciosa quando enriquecimento de perfil ou status do modelo falham, sem quebrar o fluxo principal do showcase
...
**Por que P1:** O carrinho é a primeira superfície do novo fluxo principal. Se ele aceita estados inválidos, a narrativa `Carrinho -> Pedido -> Treino` perde credibilidade logo no primeiro passo.
...
**Por que P1:** O painel de análise perde valor pedagógico quando o perfil do cliente mostra `0 pedidos` e `Sem pedidos registrados` para todos os casos, apesar de os dados já existirem na API.
...
**Por que P1:** O objetivo do projeto é explicar o ciclo de aprendizado. Se `Pos-Efetivar` não muda e a UI não explica por quê, o valor didático do showcase é perdido exatamente no momento mais importante.
...
|---|---|---|---|
| INTEG-01 | P1: Validar disponibilidade por país em `POST /carts/{clientId}/items` | Design | Pending |
| INTEG-02 | P1: Rejeição não altera o carrinho ativo | Design | Pending |
| INTEG-03 | P1: Produto compatível continua adicionável | Design | Pending |
| INTEG-04 | P1: Upsert compatível preserva incremento de quantidade | Design | Pending |
| INTEG-05 | P1: Frontend desabilita `Adicionar ao Carrinho` para item incompatível | Design | Pending |
| INTEG-06 | P1: UI expõe motivo explícito do bloqueio por país | Design | Pending |
| INTEG-07 | P1: Motivo `sem cliente` continua distinto de `produto incompatível` | Design | Pending |
| INTEG-08 | P1: Mensagem de erro inválido propaga de forma consistente no frontend | Design | Pending |
| INTEG-09 | P1: Carrinho legado/inválido bloqueia checkout até correção | Design | Pending |
| INTEG-10 | P1: Backend permanece fonte de verdade em divergência stale | Design | Pending |
| INTEG-11 | P1: Seleção de cliente dispara `GET /clients/{id}` | Design | Pending |
| INTEG-12 | P1: Seleção de cliente dispara `GET /clients/{id}/orders` | Design | Pending |
| INTEG-13 | P1: `ClientProfileCard` mostra loading/skeleton durante enriquecimento | Design | Pending |
| INTEG-14 | P1: Estado do cliente incorpora `totalOrders`, `totalSpent`, `lastOrderAt` | Design | Pending |
| INTEG-15 | P1: Estado do cliente incorpora até 5 produtos recentes | Design | Pending |
| INTEG-16 | P1: Cliente sem pedidos mostra estado vazio verdadeiro | Design | Pending |
| INTEG-17 | P1: Falha parcial de enriquecimento não quebra o fluxo principal | Design | Pending |
| INTEG-18 | P1: Falha total de enriquecimento mantém cliente selecionado e fallback local | Design | Pending |
| INTEG-19 | P1: Resposta stale não sobrescreve cliente mais recente | Design | Pending |
| INTEG-20 | P1: `ClientProfileCard` renderiza estatísticas e produtos recentes reais | Design | Pending |
| INTEG-21 | P1: `ModelStatusPanel` usa copy orientada ao avaliador | Design | Pending |
| INTEG-22 | P1: Estado `promoted` explica nova versão ativa em `Pos-Efetivar` | Design | Pending |
| INTEG-23 | P1: Estado `rejected` explica manutenção do modelo atual e motivo | Design | Pending |
| INTEG-24 | P1: `Pos-Efetivar` exibe aviso explícito quando reutiliza o modelo atual | Design | Pending |
| INTEG-25 | P1: Estado `failed` explica ausência de novo snapshot pós-checkout | Design | Pending |
| INTEG-26 | P1: Estado `unknown` mantém narrativa coerente e CTA de refresh | Design | Pending |
| INTEG-27 | P1: Retrain manual permanece claramente secundário/diagnóstico | Design | Pending |
| INTEG-28 | P1: Terminologia visual permanece alinhada a `Com Carrinho -> Pos-Efetivar` | Design | Pending |
| INTEG-29 | P1: Copy desambigua ausência de mudança visual vs bug | Design | Pending |
| INTEG-30 | P1: Painel + feedback da coluna tornam o resultado inteligível sem logs | Design | Pending |
...
- `api-service/src/main/java/com/smartmarketplace/service/CartApplicationService.java` já carrega `Client` e `Product` em `addItem()`, mas ainda não valida `product.getCountries()` contra `client.getCountry()`
- `api-service/src/main/java/com/smartmarketplace/service/OrderApplicationService.java` já possui a regra de disponibilidade por país no momento de criar `Order`, o que indica que a validação precisa apenas ser antecipada para a entrada no carrinho
- `frontend/components/catalog/CatalogPanel.tsx` atualmente só usa `cartActionDisabledReason` para o caso "Selecione um cliente para usar o carrinho"; ainda não existe razão específica para incompatibilidade por país
- `frontend/components/layout/ClientSelectorDropdown.tsx` ainda converte o cliente listado com `totalOrders: 0` e `recentProducts: []`
- `api-service/src/main/java/com/smartmarketplace/service/ClientApplicationService.java` já expõe `getClient()` com `PurchaseSummaryDTO` e `listClientOrders()` com pedidos paginados, então o backend necessário para enriquecimento principal já existe
- `frontend/lib/types.ts` define `Client` apenas com `totalOrders` e `recentProducts`; `totalSpent` e `lastOrderAt` ainda não fazem parte do shape consumido pela UI
- `frontend/components/client/ClientProfileCard.tsx` hoje só mostra total de pedidos e produtos recentes; ainda não há estado de loading nem campos para valor gasto e última compra
- `frontend/components/retrain/ModelStatusPanel.tsx` e `frontend/lib/hooks/useModelStatus.ts` já implementam a máquina de estados principal, mas a copy do painel ainda é genérica e a explicação do caso `rejected` não se estende claramente para a leitura de `Pos-Efetivar`
- `frontend/lib/fetch-wrapper.ts` já propaga `body.message`, então mensagens coerentes do backend podem chegar ao frontend sem uma camada extra de tradução

### Implicações para a próxima fase

- O design do `M15` deve definir como a UI comunica incompatibilidade de país sem competir com os badges existentes do catálogo
- O design deve escolher a estratégia de enriquecimento assíncrono do cliente selecionado, incluindo proteção contra respostas stale
- O design deve decidir onde o aviso "sem mudança visível porque o modelo atual foi mantido" vive em `Pos-Efetivar` para complementar, sem duplicar em excesso, o `ModelStatusPanel`

---

## Critérios de Sucesso

- [ ] Um cliente `BR` não consegue adicionar ao carrinho um produto fora de `BR`, e o motivo é compreensível tanto antes quanto depois de um request forçado
- [ ] O `ClientProfileCard` deixa de exibir falso vazio para clientes com histórico e passa a refletir dados reais de pedidos
- [ ] O fluxo principal de catálogo, carrinho, checkout e recomendações continua utilizável mesmo se o enriquecimento de perfil falhar
- [ ] Os estados `promoted`, `rejected`, `failed` e `unknown` deixam de parecer ambíguos para o avaliador na aba `Análise`
- [ ] O caso "sem mudança visível" em `Pos-Efetivar` passa a ser explicado explicitamente, em vez de parecer defeito da UI
# M15 — Cart Integrity & Comparative UX — Especificação
