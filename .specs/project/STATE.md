...
*Last updated: 2026-05-05 — **M16** ✅; **M17** ✅ **COMPLETE** (P1+P2+P3); **M18** (AD-055) ✅; **M19** ✅ **IMPLEMENTED** (2026-05-01); **M20** ✅ **IMPLEMENTED**; **M21** em implementação parcial (**faltam P3/P4/P5**) — [spec M21](../features/m21-ranking-evolution-committee-decisions/spec.md). **M22** (torre de item híbrida denso+esparsa, cold start) — **IMPLEMENTED** (`ai-service` core, flags default off, 2026-05-02): [ADR-074](../features/m22-hybrid-dual-item-tower-cold-start/adr-074-m22-milestone-hybrid-sparse-item-tower.md), [spec M22](../features/m22-hybrid-dual-item-tower-cold-start/spec.md). **M23:** **IMPLEMENTED** no `ai-service` (2026-05-04, `legacy` default, benchmark/rollout docs prontos) — [RFC M23](../features/m23-negative-sampling-soft-hard-ranking/rfc.md), [spec M23](../features/m23-negative-sampling-soft-hard-ranking/spec.md), [design M23](../features/m23-negative-sampling-soft-hard-ranking/design.md), [tasks M23](../features/m23-negative-sampling-soft-hard-ranking/tasks.md). **M24:** **IMPLEMENTED** no `api-service` (2026-05-05, `mvn verify` verde) — [RFC M24](../features/m24-relational-consistency-and-query-performance/rfc.md), [spec M24](../features/m24-relational-consistency-and-query-performance/spec.md), [design M24](../features/m24-relational-consistency-and-query-performance/design.md), [tasks M24](../features/m24-relational-consistency-and-query-performance/tasks.md). Próximo foco: **M21 P3/P4/P5**.*

---

## Current Focus

**Current planning:** **M17** ✅ concluído (P1+P2+P3). **M20** ✅ implementado. **M21** segue em execução parcial com pendências **P3/P4/P5**. **M18 — Catálogo simplificado (AD-055)** ✅ **COMPLETE** (2026-04-30) — [spec M18](../features/m18-catalog-simplified-ad055/spec.md). **M24** ✅ implementado e verificado em `api-service`; foco seguinte permanece em **M21 P3/P4/P5**. E2E `[m18-catalog-ad055.spec.ts](../../frontend/e2e/tests/m18-catalog-ad055.spec.ts)`.

**M17 — ✅ COMPLETE (P1+P2+P3)** — [ADR-062](../features/m17-phased-recency-ranking-signals/adr-062-phased-recency-ranking-signals.md) (re-rank, `rankScore` / `recencySimilarity`); [ADR-063](../features/m17-phased-recency-ranking-signals/adr-063-score-breakdown-api-and-product-detail-modal.md) + [ADR-064](../features/m17-phased-recency-ranking-signals/adr-064-rankingconfig-zustand-recommendation-slice.md) (`rankingConfig`, modal). Operador: [ai-service/README.md](../../ai-service/README.md). **M16 — ✅ COMPLETE** (2026-04-30).
...
| **P3** | Concluído                                  | **M17 Fase 3** — atenção temporal                                                                                                                                          | [spec M17 P3](../features/m17-phased-recency-ranking-signals/spec.md); PRS-14–15                                                                                                                                                                                                                                                                            | Fechado                                                                                     |
...
| **—**  | **IMPLEMENTED**                            | **M20** — manual retrain + métricas + Pos-Retreino (ADR-067)                                                                                                               | [spec M20](../features/m20-manual-retrain-metrics-pos-retreino/spec.md); [design M20](../features/m20-manual-retrain-metrics-pos-retreino/design.md); [ADR-067/068/069](../features/m20-manual-retrain-metrics-pos-retreino/adr-067-manual-retrain-metrics-showcase-pos-retreino.md); [tasks](../features/m20-manual-retrain-metrics-pos-retreino/tasks.md) | Monitorização e ajustes finos                                                               |
| **—**  | **IMPLEMENTAÇÃO PARCIAL**                  | **M21** — ranking / perfil / híbrido (ADR-070 + ADR-071)                                                                                                                   | [spec M21](../features/m21-ranking-evolution-committee-decisions/spec.md); [design](../features/m21-ranking-evolution-committee-decisions/design.md); [ADR-071](../features/m21-ranking-evolution-committee-decisions/adr-071-m21-neural-head-and-pure-fusion-boundary.md); [tasks](../features/m21-ranking-evolution-committee-decisions/tasks.md)         | Concluir **P3/P4/P5**                                                                       |
| **—**  | **IMPLEMENTED** (`ai-service`, 2026-05-02) | **M22** — torre de item híbrida (HF + esparsa) cold start ([ADR-074](../features/m22-hybrid-dual-item-tower-cold-start/adr-074-m22-milestone-hybrid-sparse-item-tower.md)) | [spec M22](../features/m22-hybrid-dual-item-tower-cold-start/spec.md); [design M22](../features/m22-hybrid-dual-item-tower-cold-start/design.md); [tasks](../features/m22-hybrid-dual-item-tower-cold-start/tasks.md)                                                                                                                                       | **Operador:** `M22_*` + treino + `precisionAt5`; rollback no README; não substitui M21      |
| **—**  | **IMPLEMENTED** (`ai-service`, 2026-05-04) | **M23** — redesenho de negative sampling soft + hard para ranking                                                                                                          | [RFC M23](../features/m23-negative-sampling-soft-hard-ranking/rfc.md); [spec M23](../features/m23-negative-sampling-soft-hard-ranking/spec.md); [design M23](../features/m23-negative-sampling-soft-hard-ranking/design.md); [tasks M23](../features/m23-negative-sampling-soft-hard-ranking/tasks.md)                                                                                              | `benchmark:m23` + rollout controlado; manter `legacy` até validação                         |


Decisão explícita da ordem P1→P2: **[AD-056](#state-ad-056)**.

**Ao retomar:** **M17** ✅ fechado; **M20** ✅ implementado; **M21** — concluir **P3/P4/P5**; **M23** ✅ implementado no `ai-service` com `legacy` default, `benchmark:m23` e playbook de rollout/rollback; **M18** ✅ fechado; **M19** ✅ fechado (2026-05-01); débito técnico opcional: **AD-053** / [ADR-053](../features/m12-self-healing-model-startup/adr-053-tech-debt-migrate-seed-to-api-service.md).
...
### ADR-074: Milestone M22 — torre de item híbrida (denso HF + esparsa) para cold start (2026-05-02)

**Decisão:** Abrir milestone **M22** separado de M21: representação de item em **três vias** (HF semântico; prior estrutural esparsa **sem** `product_id`; embedding opcional de **product_id** só memorização), fusão explícita **f(u, e_sem, e_struct, e_id)**, flags e gate `precisionAt5`, sem revogar M17 P3 nem as faixas M21. **Revisão comité 2026-05-02** incorporada na ADR-074.

**Registo completo:** [ADR-074](../features/m22-hybrid-dual-item-tower-cold-start/adr-074-m22-milestone-hybrid-sparse-item-tower.md).

**Estado:** Accepted (revisão decomposição A/B/C).

---

### AD-075: Marco M23 aberto com RFC — negative sampling soft + hard para ranking (2026-05-04)

**Decisão:** Abrir o marco **M23** com RFC dedicada para redesenho da estratégia de negativos no treino de ranking. A RFC define a hipótese central: reduzir exclusões amplas e tratar negativos semelhantes como sinal principal de aprendizagem (hard negatives), com limpeza restrita a quase-duplicatas.

**Artefactos canónicos:** [RFC M23](../features/m23-negative-sampling-soft-hard-ranking/rfc.md), [spec M23](../features/m23-negative-sampling-soft-hard-ranking/spec.md), [design M23](../features/m23-negative-sampling-soft-hard-ranking/design.md) e [tasks M23](../features/m23-negative-sampling-soft-hard-ranking/tasks.md).

**Razão:** O comitê convergiu que o núcleo do problema de ranking está em escolher entre alternativas semelhantes; a política atual de exclusão de semelhantes protege contra falso negativo, mas limita aprendizagem intra-categoria e degrada cenários de cold start.

**Trade-off:** Potencial aumento de ruído de treino e maior variância entre runs; mitigação prevista via thresholds conservadores de exclusão, rollout por feature flag, benchmark repetido e gate de promoção com protocolo estável.

**Impacto esperado:** Melhor sinal de ranking em itens próximos, menor incidência de neural score colapsado, e base técnica para execução controlada do plano T23 (`legacy` vs `stratified`) no fluxo `/tlc-spec-driven`.

**Estado:** Recorded ✓ + Specified ✓ + Designed ✓ + Tasks Ready ✓ + Executed / IMPLEMENTED ✓ (2026-05-04).
...
- **Pos-Comite 2026-04-29 (AD-054) — `specify feature` (M16):** `spec.md` criado em `.specs/features/m16-neural-first-didactic-ranking-catalog-density/spec.md` com 38 requisitos (`NFD-01..NFD-38`) cobrindo cooldown de compras recentes, painel `Compras recentes`, badges de elegibilidade, separacao `Vitrine x Ranking IA`, bloco "o que mudou no modelo" e seed/dataset mais denso por categoria.
- **Pos-Comite 2026-04-29 (AD-054) — `design feature` (M16):** `design.md` aprovado; desenho técnico e ADRs 055–061 executados no código.
- **Pos-Comite 2026-04-29 (AD-054) — Re-baseline do dataset/metricas:** aceite como concluído para fecho M16 (T15 / `precisionAt5` e auxiliares conforme processo acordado no repositório).
- **Pos-Comite 2026-04-29 (AD-054) — `execute` / fecho M16:** código + testes (`tasks.md` T1–T15); ADR-061 no caminho checkout→Neo4j.
- `**plan feature` P1 — M4 recência (ADR-062):** milestone **M17** no `ROADMAP.md` + pasta `.specs/features/m17-phased-recency-ranking-signals/spec.md`. **M17 concluído (P1+P2+P3)** — ver [spec M17](../features/m17-phased-recency-ranking-signals/spec.md) e [tasks M17](../features/m17-phased-recency-ranking-signals/tasks.md).
- `**plan feature` P2 — AD-055 catálogo / payload:** milestone **M18** no `ROADMAP.md` + pasta `.specs/features/m18-catalog-simplified-ad055/spec.md`. `**specify feature` ✅** — requisitos `CSL-01..CSL-11`, reconciliação `NFD-*`. `**design.md` ✅** — [design](../features/m18-catalog-simplified-ad055/design.md). `**tasks.md` ✅** — [tasks](../features/m18-catalog-simplified-ad055/tasks.md) (T1…T9). `**execute` ✅** — T1…T9; E2E `m18-catalog-ad055.spec.ts`; ADR-055/056/058. Ver [AD-055](#state-ad-055) e [AD-056](#state-ad-056).
- **Pos-Comite 2026-04-28 (AD-043/044/045) — `plan features`:** roadmap reorganizado em torno do fluxo `Carrinho -> Pedido -> Treino` + ancora visual `ModelStatusPanel`. Proximos milestones: **M13 — Cart, Checkout & Async Retrain Capture** | **M14 — Catalog Score Visibility & Cart-Aware Showcase** | **M15 — Cart Integrity & Comparative UX**. `ROADMAP.md` atualizado e features marcadas como `PLANNED`.
- **Pos-Comite (AD-043/044/045) — `specify feature` (M13):** "Cart, Checkout & Async Retrain Capture". 68 reqs (CART-01..CART-68). Frequencia de retrain decidida como `every_checkout` no MVP. `spec.md` criado em `.specs/features/m13-cart-checkout-async-retrain/spec.md`.
- **Pos-Comite (AD-043/044/045/046) — `task feature` (M13):** `tasks.md` do M13 foi realinhado ao design final (arquitetura + UI). Plano agora contem 20 tarefas com `CartSummaryBar`, semantica real de `TabNav`, acessibilidade/motion, queue semantics + `afterCommit`, gates `build` ao fim de cada fase e E2E final `m13-cart-async-retrain`.
...
- **Resolvido — Renomear vocabulario no frontend:** `Demo Comprar` -> `Adicionar ao Carrinho`, `Limpar Demo` -> `Esvaziar Carrinho`, `Com Demo` -> `Com Carrinho`, `demoSlice` -> `cartSlice`. O fluxo principal foi migrado e os restos legados de frontend ligados a `demo` foram removidos na reconciliacao de 2026-04-29.
- **Resolvido — `RetrainPanel -> ModelStatusPanel`:** imports, testes E2E e referencias principais foram migrados para a ancora visual final do fluxo pos-checkout.
- **Resolvido — `useRetrainJob -> useModelStatus`:** fonte de verdade trocada de `jobId` para `version`, preservando polling e integracao com `/model/status`.
- **Resolvido — Botao "Retreinar Modelo" no modo `Avancado`/`modo demo`:** retrain manual permanece como affordance secundaria de diagnostico dentro do `ModelStatusPanel`, fora do fluxo principal de carrinho/checkout.
- **Resolvido — Frequencia de retrain pos-checkout (gray area):** decisao final registrada no `spec.md` do M13 como `every_checkout`; `POST /carts/{clientId}/checkout` retorna `expectedTrainingTriggered: true` para carrinho nao-vazio no MVP.
...
- **M11 quick fix (ADR-031):** `supplierId?: string` adicionado ao `ProductDTO`; filtro soft negatives em `buildTrainingDataset` (exclusão de categoria+supplierName); 2 novos testes unitários; ESLint ✓; 74/74 Vitest ✓. Commit: `fix(ai-service): exclude soft negatives by category+supplier to prevent gradient interference (ADR-031)`
- **M11 quick fix (ADR-032):** `cosineSimilarity` pura adicionada a `training-utils.ts`; filtro `softPositiveIdsBySimilarity` (threshold via `SOFT_NEGATIVE_SIM_THRESHOLD`, default 0.65) aplicado após ADR-031; 2 novos testes (exclusão por cosine + threshold=1.0 desabilitado); ESLint ✓; 76/76 Vitest ✓. Commit: `fix(ai-service): add cosine similarity soft negative filter to complement ADR-031 (ADR-032)`
- **Specify M12:** `spec.md` criado para Self-Healing Model Startup (12 reqs, M12-01..M12-12)
- **Design M12:** `design.md` criado + ADR-033 (`StartupRecoveryService`) + ADR-034 (`/ready` probe + compose startup cycle); proximo: tasks
- **Tasks M12:** `tasks.md` criado (T1..T6) com dependências, gates e traceability 12/12
- **Execute M12:** T1..T6 concluídas; `TrainingJobRegistry.waitFor()`, `StartupRecoveryService`, `AUTO_HEAL_MODEL`, bootstrap testável (`startup.test.ts`), compose `/ready` + `start_period: 180s`, build gate (`lint + build + test`) e validação cold/warm boot com recomendações funcionando sem chamada manual de geração/treino
- Specify M1 features (monorepo structure, seed, Neo4j schema) — spec.md created (28 reqs, M1-01..M1-28)
- Design complex M1 — design.md + ADR-001 (seed strategy) + ADR-002 (Neo4j healthcheck) created
- Break M1 into tasks — tasks.md created (21 tasks, 6 phases, 28/28 reqs mapped)
- Execute M1 — all 21 tasks complete, all 5 services healthy, seed idempotent, 28/28 requirements met
- Specify M2 features (Spring Boot API endpoints)
- Execute M2 — 45 Java classes implemented (controllers/services/repositories/entities/config/exception), OpenAPI + Actuator + cache + recommendation fallback validated via runtime smoke tests
- Specify M3 features (AI service embedding + RAG) — spec.md created (37 reqs, M3-01..M3-37)
- Design complex M3 — design.md + ADR-003 (estrutura modular) + ADR-004 (driver singleton) + ADR-005 (warm-up + liveness/readiness) criados; 3 nós ToT, committee review com 3 personas, 6 findings incorporados
- Break M3 into tasks — tasks.md (13 tasks, T0..T13)
- Execute M3 — 13 tasks complete, tsc --noEmit clean, all 37 requirements met
- Specify M4 features (neural model + hybrid recommendation) — spec.md criado (34 reqs, M4-01..M4-34)
- Design complex M4 — design.md + ADR-006 (ModelStore atomic swap) + ADR-007 (batch predict tensor strategy) + ADR-008 (tf.tidy async boundary) criados; 3 nós ToT, committee review com 3 personas, 7 findings incorporados
- Break M4 into tasks — tasks.md (9 tasks, T1..T9)
- Execute M4 — 9 tasks complete, tsc --noEmit clean, 34/34 requirements verified ✅ COMPLETE
- Specify M5 features (Next.js frontend) — spec.md criado (33 reqs, M5-01..M5-33)
- Design M5 — design.md + ADR-001..ADR-004 criados
- Break M5 into tasks — tasks.md (40 tasks, 8 phases, 33/33 reqs mapped)
- Execute M5 — 40 tasks complete, `npm run build` ✓, `npm run lint` ✓ zero warnings, 33/33 requirements met
- Specify M6 features (tests + README) — spec.md criado (35 reqs, M6-01..M6-35)
- Design complex M6 — design.md + ADR-009 (Vitest DI mocking) + ADR-010 (xenova pre-download builder stage) + ADR-011 (Next.js standalone Dockerfile) criados; 3 nós ToT, committee review com 3 personas, 9 findings incorporados
- Break M6 into tasks — tasks.md (19 tasks, 7 phases, 55+ reqs mapped)
- Execute M6 — 19 tasks complete; 19 AI service tests (Vitest); 15 Java unit tests (JUnit 5); Testcontainers IT tests; multi-stage Dockerfiles; ai-model-data volume; bilingual README; CONTRIBUTING; ESLint ✓; Checkstyle ✓ 0 violations; M6 ✅ COMPLETE
- Specify M7 features (production readiness) — spec.md criado (36 reqs, M7-01..M7-36); 5 features (GAP-02, async train, cron GAP-01, model versioning, security + E2E)
- Design complex M7 — design.md + ADR-012 (TrainingJobRegistry) + ADR-013 (VersionedModelStore) + ADR-014 (admin key scoped plugin) + ADR-015 (AiSyncClient fire-and-forget) criados; 3 nós ToT, committee review com 3 personas, 8 findings incorporados
- Break M7 into tasks — tasks.md criado (21 tarefas, 8 fases, 37/37 reqs mapeados); Granularity ✅, Diagram-Definition ✅, Test Co-location ✅
- Execute M7 — 21 tasks complete; 42 AI service tests (Vitest: 19 existing + 23 new); 16 Java tests; ESLint ✓; Checkstyle 0 violations; Playwright E2E suite (search, recommend, rag); VersionedModelStore, TrainingJobRegistry, CronScheduler, adminRoutes, sync-product, AiSyncClient all implemented; M7 ✅ COMPLETE
- Specify M8 — UX Journey Refactor (página única, client selector na navbar, "Ordenar por IA", RAG side drawer) — spec.md criado (55 reqs, M8-01..M8-55)
- Design complex UI M8 — design.md (Approved) + ADR-017 (FLIP sem flushSync) + ADR-018 (RAGDrawer always-mounted) + ADR-019 (Zustand slices + domain hooks); 5 personas; 3 High findings incorporados
- Break M8 into tasks — tasks.md criado (14 tarefas, 6 fases, 55/55 reqs mapeados); Granularity ✅, Diagram-Definition ✅, Test Co-location ✅
- Execute M8 — 14 tasks complete; Zustand store (3 slices) + 4 domain hooks + ReorderableGrid (FLIP ADR-017) + ClientSelectorDropdown + RAGDrawer (always-mounted ADR-018) + ScoreBadge + CatalogPanel toolbar + Header wiring + layout.tsx Providers removed + ClientPanel read-only + RecommendationPanel banner + sonner toasts + E2E m8-ux-journey.spec.ts; `npm run build` ✓; ESLint ✓ 0 warnings; M8 ✅ COMPLETE
- M8 nav quick fix — abas Cliente/Recomendações removidas; nova aba Análise (ClientProfileCard + comparação Sem IA vs Com IA); ShuffledColumn migrada para useSelectedClient; antecipa estrutura prevista no M9-B; `npm run build` ✓; ESLint ✓
- Specify M9-A — Demo Buy + Live Reorder (profile vector incremental, nova rota demo-buy) — spec.md criado (33 reqs, M9A-01..M9A-33); 3 rotas mapeadas; componentes existentes reutilizáveis identificados; latência estimada 160–230ms
- Design Complex M9-A — design.md (Approved) + ADR-021 (write transaction unificada Neo4j) + ADR-022 (DELETE path params); DemoBuyService + recommendFromVector() + 3 métodos Neo4jRepository; demoSlice loading state; 4 committee findings incorporados; 3 ToT nodes; committee review 3 personas
- Break M9-A into tasks — tasks.md criado (9 tarefas, 4 fases, 33/33 reqs mapeados); Granularity ✅, Diagram-Definition ✅, Test Co-location ✅
- Execute M9-A — 9 tasks complete; DemoBuyService + recommendFromVector + Neo4jRepository (createDemoBoughtAndGetEmbeddings, deleteDemoBoughtAndGetEmbeddings, clearAllDemoBoughtAndGetEmbeddings) + demoBuyRoutes (ADR-022) + demoSlice loading state + ProductCard demo buttons/badge + CatalogPanel wiring (handlers + Limpar Demo toolbar) + 3 Next.js proxy routes + E2E m9a-demo-buy.spec.ts; ClientNotFoundError moved to Neo4jRepository; 63 AI tests (Vitest); `npm run build` ✓; ESLint ✓ 0 warnings; `tsc --noEmit` ✓; M9-A ✅ COMPLETE
- Specify M9-B — Deep Retrain Showcase — spec.md criado (32 reqs, M9B-01..M9B-32); 6 stories P1/P2/P3; 4 novos componentes (RetrainPanel, TrainingProgressBar, ModelMetricsComparison, useRetrainJob); 0 mudanças de backend; layout integrado na aba "Análise"
- Design M9-B — design.md (Approved) + ADR-023 (AnalysisPanel always-mounted) + ADR-024 (progress bar scaleX) + ADR-025 (jobIdRef stale closure); 3 proxy routes; lib/adapters/train.ts; 8 committee findings incorporados
- Break M9-B into tasks — tasks.md criado (9 tarefas, 4 fases, 32/32 reqs mapeados); Granularity ✅, Diagram-Definition ✅, Test Co-location ✅
- Execute M9-B — 9 tasks complete; lib/types.ts (5 tipos M9-B) + lib/adapters/train.ts + 3 proxy routes + useRetrainJob hook (ADR-025 jobIdRef, polling backoff, circuit-breaker 3 erros) + TrainingProgressBar (ADR-024 scaleX) + ModelMetricsComparison + RetrainPanel + AnalysisPanel lg:grid-cols-2 + mobile Tabs + page.tsx always-mounted ADR-023 + E2E m9b-deep-retrain.spec.ts; npm run build ✓; ESLint ✓ 0 warnings; M9-B ✅ COMPLETE
- M10 — Demo-Retrain Integration — Neo4jRepository.getAllDemoBoughtPairs() + ModelTrainer mescla demos no clientOrderMap (ADR-026); compras demo feitas antes do retreinamento agora participam do tensor de treino
- Break M11 into tasks — tasks.md criado (8 tarefas, 4 fases, 27/27 reqs mapeados); Granularity ✅, Diagram-Definition ✅, Test Co-location ✅
- Break M23 into tasks — `tasks.md` criado (9 tarefas, 4 fases, ordem T23-1..T23-9, `legacy` default, benchmark gate `legacy` vs `stratified`); Granularity ✅, Diagram-Definition ✅, Test Co-location ✅
- Execute M23 — 9/9 tasks concluídas; contrato env `legacy|stratified`, helpers de soft cleanup / buckets / selector, orquestração em `training-utils`, summary no `ModelTrainer`, métricas `NDCG@K` / `MRR` / pairwise / top-N, benchmark `m23SamplingBenchmark` + CLI `benchmark:m23`, README/.env/playbook alinhados e `cd ai-service && npm run verify` ✓; M23 ✅ IMPLEMENTED com `legacy` default (2026-05-04)
- Execute M11 — 8/8 tasks complete; training-utils.ts (buildTrainingDataset + hard negative mining N=4 + seed LCG + upsampling fallback + 9 unit tests) + ModelTrainer (Dense[64, relu, l2(1e-4)]→Dropout[0.2]→Dense[1], EPOCHS=30, BATCH_SIZE=16, early stopping patience=5, seedFromClientIds, ADR-027/028) + analysisSlice.ts (4-phase discriminated union: empty|initial|demo|retrained, 4 actions, ADR-029) + RecommendationColumn.tsx (empty/loading/populated, colorScheme gray/blue/emerald/violet, capturedAt, fade-in animation, ADR-030) + AnalysisPanel (snapshot orchestration: captureInitial/captureDemo/captureRetrained via useEffect chains, xl:grid-cols-4 + md:grid-cols-2 + accordion + mobile stacked, lifted useRetrainJob shared with RetrainPanel) + RetrainPanel (disabled when phase=empty, M11-26, optional retrainJob prop) + useAppStore (analysisSlice composed, resetAnalysis() encadeado no setSelectedClient, ADR-029) + E2E m11-ai-learning-showcase.spec.ts (7 testes: initial/demo/retrain/disable/reset/accordion/mobile); ESLint ✓ 0 warnings; npm run build ✓; 72 AI tests (Vitest); M11 ✅ COMPLETE
...
- **Precision@K / nDCG evaluation endpoint:** Expose recommendation quality metrics as a dedicated API endpoint. Important for production but deferred for MVP. *(Precision@K adicionada como M6-53/54 na fase de treino — este item refere-se ao endpoint dedicado de benchmarking contínuo)*
