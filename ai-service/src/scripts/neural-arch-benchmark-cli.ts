...
function parseProfiles(arg: string | undefined): NeuralArchProfile[] | undefined {
  ...
  const allowed: NeuralArchProfile[] = ['baseline', 'deep64_32', 'deep128_64']
...
function parseArgs(argv: string[]): { out?: string; profiles?: NeuralArchProfile[]; valFraction?: number } {
  ...
  return { out, profiles, valFraction }
}

async function main(): Promise<void> {
  const { out, profiles, valFraction } = parseArgs(process.argv)
